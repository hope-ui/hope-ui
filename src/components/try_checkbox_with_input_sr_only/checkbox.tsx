...
export function Checkbox<C extends ElementType = "label">(props: CheckboxProps<C>) {
  ...
  const [local, inputProps, variantProps, others] = splitProps(
    propsWithDefaults,
    ["iconChecked", "iconIndeterminate", "invalid", "class", "children"],
    [
      "ref",
      "id",
      "name",
  ...
  return (
    <Box
      ...
      {...others}
    >
      <input type="checkbox" class={inputClasses()} {...inputProps} {...ariaAttrs} />
      <span aria-hidden={true} class={controlClasses()} {...dataAttrs}>
        <Switch>
          <Match when={inputProps.indeterminate}>{local.iconIndeterminate}</Match>
          <Match when={inputProps.checked && !inputProps.indeterminate}>{local.iconChecked}</Match>
        </Switch>
      </span>
      <Show when={local.children}>
        <span class={labelClasses()} {...dataAttrs}>
          {local.children}
        </span>
      </Show>
    </Box>
