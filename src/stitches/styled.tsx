...
import { css, ElementType, HopeStyledComponentProps, systemPropNames } from ".";
import { CSSComposer } from "./stitches.config";

export function styled<
  C extends ElementType,
  Composers extends (string | Util.Function | { [name: string]: unknown })[]
>(component: C, ...composers: CSSComposer<Composers>) {
  const cssComponent = css(...(composers as any));
  ...
  const styledComponent = <T extends ElementType = C>(
    ...
    const rootClassList = () => {
      const stitchesClass = css(styleProps);
