...
**DoD for `packages/theming`:** every non-`index` source file needs a `__tests__/` test **and** a
`docs/usage/theming/<path>/<name>.md`. Leaf folders stay flat-free (tests + fixtures under `__tests__/`).
...
- **DoD:** `presets/__tests__/presets.test.ts` (bootstrap-from-registry, extend-a-preset, deep-merge
  ...
  Tailwind shorthand; empty → `""`; sanitization throw). Docs: `docs/usage/theming/presets/{presets,token-css}.md`.
