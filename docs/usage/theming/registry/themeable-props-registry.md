...
`ComponentOverride.defaultProps` express **component chrome content** (Button's `loader`/`loadingText`),
not just visual variants. It deliberately does **not** carry per-usage behavioral props (Button's
`nativeButton`/`type`) — see [the curation rule](#the-curation-rule).
...
[`ThemeablePropsOf<K>`](../preset/preset.md) handles that:
...
meaningful and safe**: recipe variants + component-level chrome content. Deliberately **excluded**:
per-instance payload content (`children`, decorators); transient UI state (`loading`, `disabled` —
defaulting these app-wide is a footgun); controlled/identity state (`open`, `value`, `id`, `ref`);
polymorphism/styling/events (`render`/`as`, `class`, `slotClasses`, `on*`); raw DOM passthrough
(`aria-*`, `data-*`); and **per-usage behavioral props** (Button's `nativeButton`/`type`). The last
describe *what a given button is* — an anchor styled as a button, a submit button in a form — not a
design-system-wide policy, so an app-wide default for them is meaningless (a preset setting
`nativeButton: false` would break every plain `<button>`). They stay component props, never themeable.
...
- [`preset`](../preset/preset.md) — `ThemeablePropsOf<K>`, `ComponentOverride.defaultProps`, and
