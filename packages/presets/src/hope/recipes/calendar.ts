...
export const calendarRecipe = tv({
  slots: {
    ...
    // `border-collapse` so the band cells sit flush. `group/grid` gates the roving focus ring on the
    // grid actually holding focus.
    grid: "group/grid w-[calc(var(--cell-size)*7)] table-fixed border-collapse",
    ...
    cell: [
      "relative rounded-md p-0 text-center",
    ...
    // no dependence on the `:focus-visible` heuristic. Zero-spec `:where(:hover)` so a later `data-*` fill
    // wins by source order (a bare `:hover` would outrank the fills and wash out the selection). The band
    // interior + preview are transparent so the cell band shows through; endpoints paint the solid pill.
    cellTrigger: [
      ...
      "data-highlighted:bg-transparent data-highlighted:text-on-selected",
      "data-selected:bg-primary data-selected:text-on-primary",
      "data-range-middle:bg-transparent data-range-middle:text-on-selected",
