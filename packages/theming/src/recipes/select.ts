 ...
 * `@hope-ui/components` `Select` consumes it via `useRecipe("select")`, and each preset
 ...
 * its own.
 *
 * Select is **two surfaces in one recipe**: a form control (the trigger, with the current value and
 * a chevron) and a measured floating popup (the card holding the option list). It carries **no**
 * color axis — v1 is a neutral control over a neutral overlay, and the only accents are the
 * transient highlight (`active`/`on-active`) and the chosen row (`selected`/`on-selected`), neither
 * of which is a variant. Its single axis is `size`, the density scale a consumer sets once on
 * `Select.Root` and which reaches the trigger *and* the popup together — the two have to agree, or a
 * `lg` control opens an `md` list.
 *
 ...
 *   base-width-beaten-by-a-variant-width override the Popover contract warns about, in exchange for
 *   an option nobody picking a Select wants. A consumer who needs a wider popup overrides the
 *   `positioner` slot.
 * - **`selectionMode`** (`single`/`multiple`/`none`) and **`modal`** — they change ARIA and behavior,
 *   not styling, so they stay component-layer props on `Select.Root`. Same argument Listbox's
 *   contract makes for its own `selectionMode`/`focusMode`.
 *
 * The empty state is **`data-placeholder:` on the `value` slot**, not a slot of its own: nothing
 * extra is rendered when the selection is empty, only styled differently, and `createComboboxValue`
 * already writes the attribute.
 *
 ...
 * recipe variants **plus** the two chrome glyphs. A strict superset of {@link SelectRecipeVariants}
 * by construction (`extends`), so it registers in `ThemeablePropsRegistry` and
 * `ThemeablePropsOf<"select">` widens the variants-only surface without dropping anything.
 *
 * Both glyphs are **flat, discrete factory keys** — never a nested `icons` map, because
 ...
 * node that would *move* if reused. Called per instance through `runIfFunction`. Mirrors Alert's
 * `{role}Icon` keys, Listbox's `checkIcon` and Calendar's `prevIcon`/`nextIcon`.
 *
 * Select is a multi-part component, so its themeable surface stays on the **root**: `Select.Root`
 * resolves both and flows them to `Select.Icon` and `Select.ItemIndicator` through context, where
 * they are the default children. The **per-instance** override is each part's own `children`.
 */
 ...
 * empty through `data-placeholder:`, which is why there is no `placeholder` slot); `icon` the
 ...
 *
 * **No `root` slot** — `Select.Root` renders no element at all, so a root class would have nothing
 * to apply to (the reason `Dialog.Root`'s and `Popover.Root`'s were removed). **No `portal` slot** —
 * same reason.
 *
 * `content` and `list` stay distinct because `role="listbox"` may only contain options and groups: a
 * future `Combobox.Empty`/`Combobox.Status` has to live in the card beside the list, not inside it.
 */
