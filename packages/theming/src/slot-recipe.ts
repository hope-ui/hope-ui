 ...
 *
 * A recipe is a `tailwind-variants` slot recipe (the `tv` re-exported from `./styling`) used
 * **directly**, with no adapter: calling it with variant props returns one class *function* per slot.
 * Each component's contract under `recipes/` specializes this with its own variants and slot names.
 */
