...
export interface ComboboxClearProps extends Omit<CreateComboboxClearProps, "onClear"> {
  ...
  /**
   * Whether to render while the field is already empty. Default `false` — a clear button with nothing
   * to clear is a control that does nothing.
   */
 ...
 * back the very text the user just asked to remove.
 *
 * **What "clear" means is this layer's call**, since the behavior layer owns no text value. Here it
 * means both halves — the text *and* the selection — because a field showing nothing while still
 * reporting `apple` to its `onChange` is exactly the mismatch `allowsCustomValue` exists to make
 * deliberate.
 */
  ...
  const clear = createComboboxClear(
    ...
    merge(omit(props, "render", "class", "children", "alwaysVisible"), {
      onClear: () => {
        ...
        // Through the selection API, so `onChange` fires and a controlled consumer hears about it —
        // never by writing the text and leaving the selection behind.
