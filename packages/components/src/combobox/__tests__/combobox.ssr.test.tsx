...
// `Tree` (from `combobox.ssr-entry.tsx`) is the single source of truth for the round-trip tree:
// `combobox.browser.test.tsx` hydrates the very same render. Hydration keys are allocated by walking
...
describe("Combobox SSR", () => {
  ...
  it("renders the input with its name and its value, and no popup", async () => {
    ...
    // both sides — which is what lets an untracked value snapshot agree across hydration.
    ...
    // otherwise every closed Combobox on the page carries an invalid IDREF.
  ...
  it("renders no hidden native form control", async () => {
    ...
    // `ComboboxRootProps`. Select renders a `<select>` carrying every `<option>` because its kernel
    // holds the whole option set; a Combobox's holds the *filtered* one, so the same field would drop
    // options as the user typed and submit whatever the query happened to leave.
  ...
  it("renders the gutter buttons outside the tab order", async () => {
    ...
    // Case-insensitive: the props ride through a spread, so the server serializes the key verbatim
    // as `createButton` spells it (`tabIndex`). HTML attribute names are case-insensitive, so the
    // parser lowercases it and the hydrated DOM agrees — the effective `element.tabIndex === -1` is
    // pinned in the browser half.
  ...
  it("renders an open Combobox on the server without crashing, and still portals nothing", async () => {
    // `Combobox.Portal` returns `null` under `isServer` — @solidjs/web's `Portal` throws server-side
    // rather than degrading — so an open Combobox must render, and render only its control.
    const html = await renderToStringAsync(() => <Tree defaultOpen />);
    expect(html).toMatch(/role="combobox"/);
    expect(html).toMatch(/aria-expanded="true"/);
    expect(html).not.toMatch(/role="listbox"/);
    expect(html).not.toMatch(/data-slot="combobox-content"/);
    // The announcer is `createComboboxStatus`'s, and it is guarded on `typeof document` — a server
    // render must not reach `document.createElement`.
  ...
  it("matches its server output byte for byte", async () => {
    ...
    // what makes `_hk` — the hydration key `combobox.browser.test.tsx` hydrates against — real.
    ...
    expect(html).toMatchInlineSnapshot(
      `"<div _hk=00y010 data-slot="combobox-control" class="relative inline-flex items-center rounded-md border border-subtle bg-surface-raised text-foreground transition-[background-color,border-color,box-shadow] duration-150 ease-out focus-within:border-focus focus-within:ring-3 focus-within:ring-focus-halo data-disabled:cursor-not-allowed data-disabled:pointer-events-none data-disabled:opacity-disabled h-9 gap-2 px-2.5 min-w-36" ><input _hk=00y0130 aria-label="Choose a fruit" placeholder="Search fruit" value="Strawberry" id="004" type="text" role="combobox" aria-autocomplete="list" aria-expanded="false" autocomplete="off" autocorrect="off" autocapitalize="none" spellcheck="false" class="min-w-0 flex-1 cursor-text border-0 bg-transparent outline-none text-foreground placeholder:text-foreground-subtle text-sm" data-slot="combobox-input" /><button _hk=00y01710 type="button" aria-label="Clear" tabIndex="-1" class="inline-flex shrink-0 items-center justify-center rounded-sm outline-none text-foreground-muted transition-[background-color,color] duration-150 ease-out hover:not-data-pressed:bg-surface-raised-hovered data-pressed:bg-surface-raised-pressed [&amp;_svg]:size-3.5" data-slot="combobox-clear" ><svg _hk=00y01711 viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg></button><button _hk=00y01a0 type="button" aria-label="Show suggestions" aria-haspopup="listbox" aria-expanded="false" tabIndex="-1" class="inline-flex shrink-0 cursor-default items-center justify-center rounded-sm select-none outline-none text-foreground-muted" data-slot="combobox-trigger" ><span _hk=00y01a20 data-slot="combobox-icon" aria-hidden="true" class="pointer-events-none inline-flex shrink-0 items-center justify-center text-foreground-muted [&amp;_svg]:size-4" ><svg _hk=00y01a21 viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m6 9 6 6 6-6"></path></svg></span></button></div>"`,
    );
  });
});
