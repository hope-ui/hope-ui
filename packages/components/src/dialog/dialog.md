...
(`@solid-zero/primitives/dialog`), which owns the entire behavior core — open state, the
...
cancel the open/close), and the `createComponentContext` plumbing that shares one `createDialog`
call across the parts. See `createDialog.md` and each internal primitive's own `.md` for the
behavior each contributes. Dialog owns none of them.
