...
describe("Badge SSR", () => {
  ...
  it("matches its server output byte for byte", async () => {
    ...
    expect(html).toMatchInlineSnapshot(
      `"<span _hk=00d0 class="inline-flex items-center justify-center whitespace-nowrap align-middle font-medium select-none border border-transparent bg-clip-padding h-5 gap-1 px-2 text-xs has-data-[slot=badge-start-decorator]:ps-1.5 has-data-[slot=badge-end-decorator]:pe-1.5 rounded-md bg-neutral-soft text-neutral-emphasis" data-slot="badge" ><span _hk=0080 data-slot="badge-label" class="inline-flex items-center">New</span></span>"`,
    );
  });
});
