...
/**
 * The title part: labels the popup. Registers its `id` on the popup's `aria-labelledby` via
 * `createRegisteredId` (which defers the ancestor-signal write past Solid 2.0's ban on writing an
 * ancestor-owned signal from a descendant's render body). Must be called from the title's own owner
 * scope, so the registration's cleanup is scoped to the title's unmount. The `createDialogTitle`
 * shape, against `createPopover`'s state.
 */
export function createPopoverTitle(
  ...
  // or the popup ends up with no `aria-labelledby` and no accessible name — which for a
