...
import type { CollectionItem } from "../create-collection/create-collection";
import type { CreateListFocusReturn } from "../create-list-focus/create-list-focus";
import type { TextDirection } from "../create-list-navigation/create-list-navigation";
...
export function createGridNavigation<V>(
  ...
  const focusCell = (cell: GridCell<V> | undefined) => {
    if (cell && focus.isFocusable(cell.item)) {
      focus.focus(cell.item);
    }
  };
    ...
    for (let guard = 0; guard <= columns * rows + columns; guard++) {
      if (col >= columns) {
        ...
        if (wrap === "loop") {
          ...
          col = 0;
          if (row >= rows) {
            return options.onNavigateAfter?.();
          }
        }
      } else if (col < 0) {
        if (wrap === "nowrap") {
        ...
        if (wrap === "loop") {
          ...
          col = columns - 1;
          if (row < 0) {
            return options.onNavigateBefore?.();
          }
        }
      }
      const target = cellAt(row, col);
      if (target && focus.isFocusable(target.item)) {
    ...
    for (let guard = 0; guard <= rows; guard++) {
      if (row >= rows) {
        if (wrap === "loop") {
          ...
          return options.onNavigateAfter?.(); // nowrap + continuous both flip at the vertical edge
        }
      } else if (row < 0) {
        if (wrap === "loop") {
          row = rows - 1;
