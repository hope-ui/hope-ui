...
Selection state layered on a [`createListFocus`](../list-focus/list-focus.md) instance: single or
...
[`@hope-ui/primitives/utils`](../../utils/equality/equality.md).
...
- **`selectionBehavior`** — `"explicit"` changes selection only on an action (Space/Enter, click);
  ...
  When combining follow with [typeahead](../list-typeahead/list-typeahead.md), pass
