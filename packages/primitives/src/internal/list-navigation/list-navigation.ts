...
export function createListNavigation<V = unknown>(
  ...
  const firstFocusable = (): number => {
    ...
    for (let index = 0; index < items.length; index++) {
      ...
      if (item && focus.isFocusable(item)) return index;
    }
    return -1;
  };
  ...
  const step = (direction: 1 | -1): number => {
    ...
    if (current < 0) return direction > 0 ? firstFocusable() : lastFocusable();
    ...
    for (let taken = 0; taken < length; taken++) {
      ...
      if (index === current) return -1; // wrapped all the way back — no other focusable item
      const item = items[index];
      if (item && focus.isFocusable(item)) return index;
    }
    return -1;
