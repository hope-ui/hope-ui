...
export function createListFocus<V = unknown>(
  ...
  const firstFocusableIndex = createMemo(() => {
    ...
    for (let index = 0; index < list.length; index++) {
      ...
      if (item && isFocusable(item)) return index;
    }
    return -1;
  });
  ...
  createEffect(
    ...
    (element) => {
      if (!element) return; // element not mounted yet — a later run fires once it is
      element.focus();
  ...
  const setActive = (index: number) => {
    ...
    if (item && item.element() == null) options.source.scrollIndexIntoView?.(clamped);
    if (focusMode() === "roving") setPendingFocus(clamped);
  };
  ...
  const getItemTabIndex = (item: CollectionItem<V>) => {
    if (focusMode() === "activedescendant" || disabled()) return -1;
    return items().indexOf(item) === rovingTabStopIndex() ? 0 : -1;
  };

  const activeDescendant = () => {
    if (focusMode() !== "activedescendant" || disabled()) return undefined;
