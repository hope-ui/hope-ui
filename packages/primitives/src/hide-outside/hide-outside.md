...
deferral already guarantees the second half. See `focus-restore.md`.
...
The count lives on the element itself under `Symbol.for("solid-zero.hide-outside")`, not in a
module-scope `WeakMap`. `@solid-zero/primitives` is public API and is depended on as a plain
