...
The catalogs `MESSAGES_EN` / `MESSAGES_FR` are exported from [`./locales/en.ts`](./locales/en.ts) and
[`./locales/fr.ts`](./locales/fr.ts) (see `locales/en.md`, `locales/fr.md`); each is typed as
