...
```tsx {5,7-11,16-18,20-21,24-29,42}
import { Box, Button, Flex, Image, Text } from "@hope-ui/core";
...
We can go a step further and add dark mode support to our `Card` component. Hope UI provides a `_dark` style prop that allows to define style when the application is in dark mode.

```tsx {12-15,26,36-39,52}
import { Box, Button, Flex, Image, Text } from "@hope-ui/core";
...
<Callout>
  Toggle the documentation color mode with the moon/sun icon at the top right to see the change on
  the `Card`.
</Callout>
