...
The `colors` key allows you to customize the color palettes for your project. Hope UI uses **10 shades** color palettes that ranges from `50` to `900`.

Two popular color palettes that you can use are:

- [TailwindCSS colors](https://tailwindcss.com/docs/customizing-colors)
- [OpenColor](https://yeun.github.io/open-color/)

<Callout>
  <span>`colors` is the only scale to have light/dark option.</span>
</Callout>
  ...
  colors: {
    light: {
      primary: {
        ...
        800: "#1e40af",
        900: "#1e3a8a",
        mainChannel: "59 130 246", // Shade 500 rgb channels
        lightChannel: "219 234 254", // Shade 100 rgb channels
        darkChannel: "30 59 138", // Shade 900 rgb channels
      },
    },
    dark: {
      primary: {
        // ...dark primary shades, if needed
      },
    },
  },
};
...
`mainChannel`, `lightChannel` and `darkChannel` **should be** space separated RGB color channels. This is used to create colors with different opacity using CSS variables.

```css
button {
  background: rgb(var(--hope-colors-primary-mainChannel) / 0.8); /* rgb(59 130 246 / 0.8) */
}
```

Below are the list of all colors available in the default Hope UI theme. Switching to dark mode will show the proper values from the
dark palette.
