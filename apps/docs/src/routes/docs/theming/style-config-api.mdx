...
We need a way to access the `baseClasses` and `styleOverrides` in `AlertTitle` and `AlertDescription` components. The easiest way to do that is by using a _context_.
