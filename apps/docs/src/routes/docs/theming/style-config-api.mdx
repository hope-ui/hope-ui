...
We need to create a _component theme_ type to get TypeScript supports in the theme object. To do that, Hope UI expose the `ComponentTheme` utility type.
...
Default props and style config can be overridden in Hope UI theme `components` section by using the **same name** passed to both `mergeThemeProps` and `useAlertStyleConfig` (here "Alert").

And to get proper TypeScript support we cast the object as an `AlertTheme` type.
...
Use the `styleConfigOverride` prop to override the style config at the component level.
