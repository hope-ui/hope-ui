...
You can create base html elements with style props support using the `hope.<element>` notation.
...
The `hope()` function converts non Hope UI components or JSX elements to _"hope-enabled"_ components, so you can use features like style props with them.
