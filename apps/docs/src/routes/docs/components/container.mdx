...
By default, the container is centered within its parent with a margin `auto` on left and right, you can disabled this behavior by setting the `isCentered` prop to `false.
