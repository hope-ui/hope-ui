...
//
// Ports hope's exact token-derivation ladder from `packages/presets/src/hope/theme.css`. Given a
...
//     `var(--color-<family>-<shade>)`, grouped/commented to mirror `theme.css`.
//
// **Correctness anchor:** `toCss(HOPE_DEFAULT_CONFIG)` reproduces the shipped `theme.css` — with one
...
// (validated against `theme.css` — feeding `HOPE_DEFAULT_CONFIG` diffs to exactly those 4 tokens).
...
// ---------------------------------------------------------------------------
// Sections — the whole token set as ordered, commented groups (mirrors theme.css).
 ...
 *
 * Defines every hope-ui semantic token (--hope-*). Import it in place of
 * @hope-ui/presets/hope/theme.css (hope's default token values), after the preset structure:
 *
 *   @import "tailwindcss";
 *   @import "@hope-ui/presets/hope/tailwind.css";
 *   @import "./theme.css";   (this file — replaces hope/theme.css)
 *
