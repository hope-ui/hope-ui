...
- **Config**: the preset’s stylesheets define, at build time, the utilities that read the `--hope-*`
  tokens (`@import "@hope-ui/presets/hope/tailwind.css"`) and their values
...
<Alert.Root
  ...
  class="not-prose"
  icon={<PaletteIcon />}
  title={<strong>Generate the token set instead of hand-writing it</strong>}
  description={
    <div>
      The{" "}
      <a
...
What matters about recipes from the outside is that presets keep them **pure** — a recipe references
finished tokens only, never a computed color. That is what makes swapping a preset safe: no recipe
can bake in a color the next preset doesn’t have.
