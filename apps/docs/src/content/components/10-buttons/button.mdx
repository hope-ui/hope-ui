...
shown [below](#on-a-tinted-surface). `inverted` is a filled variant for solid surfaces — shown
[below](#on-solid-surfaces).
...
dropped onto. `ghost` is the case that bites: its hover wash sits in the same tier as the role's
`soft` fill, so a `ghost` button inside a `soft` Alert washes to that Alert's own background and
hovering appears to do nothing at all.
...
*of* the text, over whatever is behind it. That is the same mechanism [CloseButton](/components/close-button#adapts-to-the-surface)
uses, and it is why `adaptive` ignores `colorScheme`.
