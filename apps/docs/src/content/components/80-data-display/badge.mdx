...
The `dot` variant renders role-neutral chrome with a single role-colored pip, the shape a status
indicator wants: the label carries the meaning and the color reinforces it. It works across every
role.
...
In a status column, each row states its state in words, with the pip as a scannable second channel.
...
<Alert.Root
  ...
  description={
    <div>
      ...
      link semantics, and Badge contributes its computed <code>class</code>{" "}
...
<Preview>
  ...
  <Badge
    variant="soft"
    colorScheme="success"
    class="rounded-none border-s-4 border-success"
  >
    Ribbon
  </Badge>
</Preview>
...
<PropsTable
  rows={[
    ...
    },
    {
      ...
      default: "",
      description:
        "Leading content before the label, normally an icon. Sized to the badge by the recipe.",
    },
    {
      prop: "endDecorator",
      type: "JSX.Element",
