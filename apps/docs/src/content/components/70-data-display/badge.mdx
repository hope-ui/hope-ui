...
`inverted` is the literal swap of `solid` — the on-color becomes the fill and the role becomes the
text — so it is designed to sit on a **solid, colored surface** (a toolbar, a banner) rather than
the page background. `warning` honestly inverts to a dark chip, the documented consequence of a
symmetric swap.

<Preview>
  <div class="flex flex-wrap items-center gap-3 rounded-lg bg-surface-inverse p-6">
    <Badge variant="inverted" colorScheme="primary">primary</Badge>
    <Badge variant="inverted" colorScheme="neutral">neutral</Badge>
    <Badge variant="inverted" colorScheme="success">success</Badge>
    <Badge variant="inverted" colorScheme="danger">danger</Badge>
  </div>
</Preview>
...
<Preview>
  <div class="w-72">
    <Badge variant="soft" colorScheme="info" fullWidth>Full width</Badge>
  </div>
</Preview>
...
<div class="not-prose my-6 flex gap-3 rounded-xl bg-primary-soft p-4 text-sm leading-relaxed text-primary-emphasis">
  ...
  <div>
    Badge is a static label with no behavior of its own, so — unlike Button — rendering it as an
    anchor needs no extra flags: the anchor brings its own link semantics, and Badge just contributes
    its computed <code>class</code> and merges refs. Spreading Badge's <code>span</code> props onto a
    different element may need an <code>as unknown as</code> cast to satisfy TypeScript; the runtime
...
<PropsTable
  ...
  rows={[
    { slot: "root", attr: "badge", description: "The rendered element (a span by default)." },
    { slot: "label", attr: "badge-label", description: "Wraps the children. Rendered only when children is set." },
    { slot: "startDecorator", attr: "badge-start-decorator", description: "Leading decorator wrapper. Rendered only when startDecorator is set." },
    { slot: "endDecorator", attr: "badge-end-decorator", description: "Trailing decorator wrapper. Rendered only when endDecorator is set." },
    { slot: "dot", attr: "badge-dot", description: "The status pip. Rendered only for the dot variant; marked aria-hidden." },
  ]}
/>
...
<Preview>
  <Badge variant="solid" colorScheme="primary" slotClasses={{ root: "px-4 tracking-wide uppercase" }}>
    New
  </Badge>
...
<Preview>
  ...
  >
    <Badge variant="solid" colorScheme="primary">Branded</Badge>
    <Badge variant="soft" colorScheme="success">Branded</Badge>
  </ThemeProvider>
</Preview>
  ...
  rows={[
    {
      ...
      type: "BadgeVariant",
      default: '"soft"',
      description: "Visual style: solid, inverted, soft, subtle, outline, dot. inverted is the on-color swap of solid, for solid surfaces.",
    },
    {
      prop: "colorScheme",
      type: "BadgeColorScheme",
      default: '"neutral"',
      description: "Semantic role color: primary, neutral, success, info, warning, danger. Every variant is role-aware.",
    },
    {
      prop: "size",
      type: "BadgeSize",
    ...
    },
    {
      ...
      type: "BadgeShape",
      default: '"rounded"',
      description: "Corner shape: sharp, rounded, pill, circle. circle squares the aspect for a single glyph or count.",
    },
    {
      prop: "fullWidth",
      type: "boolean",
    ...
    },
    {
      ...
      type: "JSX.Element",
      default: "",
      description: "Leading content before the label — typically an icon. Sized to the badge by the recipe.",
    },
    {
      prop: "endDecorator",
      type: "JSX.Element",
    ...
    },
    {
      ...
      default: "",
      description: "Render Badge as another element or component. Receives Badge's computed props. The only polymorphism mechanism — there is no as prop.",
    },
    {
      prop: "class",
      type: "string",
    ...
    },
    {
      ...
      default: "",
      description: "Per-slot class overrides, keyed by root, label, startDecorator, endDecorator, dot.",
    },
    {
      prop: "children",
      type: "JSX.Element",
