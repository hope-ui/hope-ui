...
export default function MenuDoc() {
  ...
  const menuPropItems: PropsTableItem[] = [
    ...
    {
      name: "children",
      description: "The children of the menu. Using a render prop gives access to the `opened` state.",
    ...
    {
      name: "offset",
      description: "Offset between the menu content (dropdown panel) and the reference (trigger) element.",
      type: "number",
    },
    {
      name: "placement",
  ...
  const menuItemPropItems: PropsTableItem[] = [
    ...
    {
      name: "onSelect",
      description: "Event handler called when the user selects a menu item (via mouse or keyboard).",
      type: "() => void",
    },
  ];
  ...
  return (
    <PageLayout previousLink={previousLink} nextLink={nextLink} contextualNavLinks={contextualNavLinks}>
      ...
      <Text mb="$5">
        An accessible dropdown menu for the common dropdown menu button design pattern. <Code>Menu</Code> implement the{" "}
        <Anchor
      ...
      <Text mb="$5">
        To access the internal state of the <Code>Menu</Code>, use a function as children (commonly known as a render
        prop). You'll get access to the internal <Code>opened</Code> state.
      </Text>
      ...
      <Text mb="$3">
        The default <Code>MenuTrigger</Code> can be styled using the usual styled-system props, but it starts off
        plainly styled.
      </Text>
      <Text mb="$12">
        You can use the <Code>as</Code> prop to render a custom component instead of the default{" "}
        <Code>MenuTrigger</Code>. For instance, you can use Hope UI's <Code>Button</Code> component, or your own custom
        component.
      </Text>
      <SectionSubtitle id="icons-and-commands">Icons and commands</SectionSubtitle>
      <Text mb="$5">
        You can add icon to each <Code>MenuItem</Code> by passing the <Code>icon</Code> prop. To add a commands (or
        hotkeys) to menu items, you can use the <Code>command</Code> prop.
      </Text>
      <Preview snippet={snippets.iconsAndCommands} mb="$10">
        <Menu>
          <MenuTrigger as={IconButton} variant="outline" colorScheme="neutral" icon={<IconHamburgerMenu />} />
          <MenuContent minW="$60">
      ...
      <Text mb="$5">
        When focus is on the <Code>MenuTrigger</Code> or within the <Code>MenuContent</Code> and you type a letter key,
        a search begins. Focus will move to the first <Code>MenuItem</Code> that starts with the letter you typed.
      </Text>
      ...
      <Alert status="warning" mb="$10">
        <AlertDescription>
          If the content of <Code>MenuItem</Code> is too complex or non-textual you can pass the <Code>textValue</Code>{" "}
          prop to <Code>MenuItem</Code> to indicate the text that will be used for typeahead purposes.
        </AlertDescription>
      </Alert>
      <SectionSubtitle id="menu-item-colors">MenuItem colors</SectionSubtitle>
      <Text mb="$5">
        Use the <Code>colorScheme</Code> prop to change the color of the <Code>MenuItem</Code>. You can set the value to{" "}
      ...
      <SectionSubtitle id="trigger-action">Trigger an action when a MenuItem is selected</SectionSubtitle>
      <Text mb="$5">
        Pass a callback to the <Code>onSelect</Code> prop to trigger an action when the <Code>MenuItem</Code> is
        selected.
      </Text>
      ...
      <Text mb="$5">
        Use the <Code>MenuGroup</Code> and <Code>MenuLabel</Code> components to group related menu items.
      </Text>
      ...
      <UnorderedList spacing="$2" mb="$8">
        ...
        <ListItem>
          <Code>MenuTrigger</Code> as <Code>aria-expanded</Code> set to <Code>true</Code> when the menu is displayed.
        </ListItem>
        <ListItem>
          <Code>MenuTrigger</Code> as <Code>aria-controls</Code> set to the id of <Code>MenuContent</Code>.
        </ListItem>
        <ListItem>
      ...
      <UnorderedList spacing="$2" mb="$8">
        <ListItem>
          <Kbd>enter</Kbd>, <Kbd>space</Kbd> or <Kbd>↓</Kbd> opens the menu and moves focus on the first menu item.
        </ListItem>
        <ListItem>
      ...
      <UnorderedList spacing="$2" mb="$8">
        <ListItem>
          <Kbd>enter</Kbd> or <Kbd>space</Kbd> activates the menu item, close the menu and sets focus to the menu
          trigger.
        </ListItem>
        <ListItem>
        ...
        <ListItem>
          <Kbd>↑</Kbd> moves focus to the previous menu item. If focus is on the first menu item, moves focus to the
          last menu item.
        </ListItem>
        <ListItem>
          <Kbd>↓</Kbd> moves focus to the next menu item. If focus is on the last menu item, moves focus to the first
          menu item.
        </ListItem>
        <ListItem>
      ...
      <UnorderedList spacing="$2" mb="$12">
        <ListItem>
          Any printable characters move visual focus to the first menu item that matches the typed character.
        </ListItem>
        <ListItem>
          If multiple keys are typed in quick succession, visual focus moves to the first menu item that matches the
          full string.
        </ListItem>
        <ListItem>
          If the same character is typed in succession, visual focus cycles among the menu items starting with that
          character.
        </ListItem>
      </UnorderedList>
      <SectionTitle id="theming">Theming</SectionTitle>
      <Text mb="$5">
        <Code>Menu</Code> base styles and default props can be overridden in the Hope UI theme configuration like below:
      </Text>
      ...
      <Text>
        <Code>MenuTrigger</Code>, <Code>MenuContent</Code>, <Code>MenuGroup</Code> and <Code>MenuLabel</Code> composes{" "}
