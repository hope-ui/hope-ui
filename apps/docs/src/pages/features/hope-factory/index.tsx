  ...
  return (
    <PageLayout previousLink={previousLink} nextLink={nextLink} contextualNavLinks={contextualNavLinks}>
      ...
      <Text mb="$5">
        ...
        <strong>function that can be used to enable custom component</strong> receive hope's style props.
      </Text>
      <CodeSnippet snippet={snippets.importHopeFactory} mb="$12" />
      <SectionTitle id="hope-jsx-elements">Hope JSX elements</SectionTitle>
      <Text mb="$5">
        Create base html elements with theme-aware style props using <Code>hope.&lt;element&gt;</Code> notation. For
        example, if you want a plain html button with ability to pass Hope UI styles, you can write{" "}
        <Code>&lt;hope.button /&gt;</Code>.
      </Text>
      ...
      <Text mb="$8">
        This reduces the need to create custom component wrappers and name them. This syntax is available for any html
        elements.
      </Text>
      <SectionTitle id="hope-factory-function">Hope factory function</SectionTitle>
      <Text mb="$5">
        This is a function that converts <strong>non-hope components</strong> or <strong>jsx element</strong> to
        hope-enabled components so you can pass style props to them.
      </Text>
      <Text mb="$5">
        Consider the <Code>Link</Code> component of the <Code>solid-app-router</Code> package, let's use the hope
        factory function to make possible to pass style props to it.
      </Text>
      <CodeSnippet snippet={snippets.hopeFactoryFunction} mb="$5" />
      <Alert status="warning" mb="$10">
        <AlertDescription>
          Considering that Hope UI uses <Code>stitches</Code> under the hood, ensure the non-hope component accepts{" "}
          <Code>class</Code> as props for this to work correctly
        </AlertDescription>
      </Alert>
      ...
      <Text>
        Taking the above example, in stitches <Code>css()</Code> method or Hope UI <Code>css</Code> prop, if you want to
        target all <Code>p</Code> in the <Code>Card</Code> component you can use the css selector{" "}
        <Code>{"${Card} p"}</Code> and it will evaluate to <Code>{".my-card p"}</Code>. If you want to learn more, check
        out "Targeting other SolidJS components" in the{" "}
        <Anchor
          as={Link}
