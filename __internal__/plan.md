...
- `mergeProps`/`splitProps` are gone; the 2.0 idiom is `merge`/`omit` from `solid-js`.
  ...
  `withDefaults` (`__internal__/primitives/utils/defaults/defaults.md`). See CLAUDE.md.
...
- Confirmed a genuine, unavoidable type-system limit for the render-prop/`as` pattern:
  ...
  trying to avoid). Documented in `__internal__/primitives/utils/render/render.md` rather than
