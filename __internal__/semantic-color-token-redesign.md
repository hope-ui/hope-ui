...
`color-mix`, `ring-focus/50`, `data-disabled:opacity-90`. **The docs (`__internal__/theming.md`, the
...
> `data-pressed:translate-y-px`. See `__internal__/theming.md` and the recipe for the current shape.
...
- `__internal__/theming.md` — rewrite **Decision 01** (`-soft` not `-subtle`), **Decision 02** (recipes never
...
- The doc website (`apps/docs/`) — the authoritative token list, the styling token names
  (`primary-hover` → `primary-hovered`, `primary-outline` → `primary-line`, add representative new
  ones), and the Button role tokens (`on-{role}-soft` → `{role}-emphasis`) now live there rather
  than in per-symbol repo docs; update them in the site.
