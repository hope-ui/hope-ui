...
virtual mode the `id` is generated by the source per row (`createItemIds`), so
`aria-activedescendant` names a stable id even for a row that is about to scroll into view.
