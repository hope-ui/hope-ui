...
doc; per-symbol API detail lives in the doc website (`apps/docs/`), and
...
- **Config** (imported into the consumer's Tailwind v4 entry CSS; peer dep `tailwindcss`):
  `@hope-ui/presets/{hope,…}` — each preset ships two CSS entries (`@import "@hope-ui/presets/hope/tailwind.css"`
  for the `@theme inline` mapping, then `@import "@hope-ui/presets/hope/theme.css"` for the `--hope-*`
  token values — the latter an opt-out import you can replace with your own). `hope`
...
- **`defaultProps` + `ThemeablePropsRegistry`** — a preset's per-component `defaultProps` (in
  ...
  **themeable-props surface**: recipe variants **plus** component chrome content (for Button,
  ...
  precedence (`useDefaults`). See the `ThemeablePropsRegistry` reference in the doc website
  (`apps/docs/`).
...
A preset ships **two Tailwind v4 CSS entries** the consumer imports, in order, into their Tailwind
entry: `@import "@hope-ui/presets/hope/tailwind.css"` (structure) then
...
restyle every token without shipping hope's defaults as dead weight; `tailwind.css` deliberately does
**not** import it. Behind those entries the CSS is split into single-responsibility files, several
**shared verbatim by every preset**, so a preset only authors what is actually preset-specific:
...
└── hope/
    ...
    ├── tailwind.css      # structural CSS entry (@hope-ui/presets/hope/tailwind.css) — @import _base/base.css, radius scale, @source recipes
    ├── theme.css         # token-value entry (@hope-ui/presets/hope/theme.css) — hope's --hope-* values (:root + .dark), a separate opt-out import
...
`SEMANTIC_OPACITY_TOKENS`) — byte-identical in every preset — so they live once in `_base/` (behind
its single `base.css` aggregator) rather than being copy-pasted per preset. The only thing a preset
authors is its token *values* (hope authors them in **CSS** — `theme.css`'s `:root`/`.dark` `--hope-*`
declarations) and its `recipes/`. Within `tailwind.css` the `@import` order is cosmetic — Tailwind
at-rules (`@theme`, `@custom-variant`) are collected at build time and `:root`/`.dark` custom
properties resolve by the cascade at use time, so nothing there is order-sensitive; the one ordering
that matters is that the consumer imports `theme.css` *after* `tailwind.css`. (`_base/` is a CSS
...
its own `theme.css` (so `<ThemeProvider>` renders no DOM — hope is a zero-DOM preset), (b) maps them
...
`_base/_opacity.css` (`@utility`, since Tailwind v4 has no `--opacity-*` namespace), and
...
which a preset runs against its token CSS — for hope, its `theme.css` (read as a string in
...
preset's `theme.css` (`focus-halo`, `scrim`). (4) The overridability unit is `(role × variant ×
...
`_base/_opacity.css`'s `@utility` layer, and exist so a recipe never hardcodes a magic `opacity-90`
...
`opacity-disabled`, `opacity-loading` (via `_base/_opacity.css`). A disabled control has no fill
